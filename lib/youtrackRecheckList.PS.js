...
if (window.location.href.includes("abilitynet.myjetbrains.com/youtrack/issues/") || window.location.href.includes("abilitynet.myjetbrains.com/youtrack/search")) {
    if (sL = prompt("Paste issue ID list (line-, comma-, space- or tab-separated), or submit empty to retrieve all issues currently shown on this page"), null != sL) {
        ...
        aR = [];
        if (3 > sL.length)
            for (var k = 0; k < aI.length; k++) aR.push(aI[k].innerText);
        else aR = sL.split(/\r?\n|\r|\n| |,|\t/g), aR = aR.filter(function(a) {
            return /[a-zA-Z0-9]/g.test(a)
        });
        for (var h = 0; h < aC.length; h++) aC[h].title.startsWith("Pages") ? aP.push(aC[h].innerText) : aC[h].title.startsWith("Devices") && aD.push(aC[h].innerText.replace("Mobile (iOS)", "iOS").replace("Mobile (Android)", "Android").replace("iPad (iOS)", "iPad"));
        for (var i = 0; i < aR.length; i++) {
            ...
            if (0 > iI) sE += sR + ": not found on this page\n";
            else {
        ...
        sT += "</table>";
        var wN = window.open("", "NW", "width=400,height=400,resizable=1"),
            content = sO;
        ...
        wN.document.close();
        var oS = wN.document.getElementById("oO"),
            oE = wN.document.getElementById("endS");
